...
import { computeLocalityScore } from '../lib/aggregate';

const router = Router();

interface LocalityScoreQuery {
  lat?: string;
  lng?: string;
  persona?: string;
  builderId?: string;
 ...
 */
router.get('/score', async (req: Request<{}, {}, {}, LocalityScoreQuery>, res: Response): Promise<void> => {
  try {
    ...
    const result = await computeLocalityScore({
      lat: latNum,
      lng: lngNum,
      persona,
      builderId,
